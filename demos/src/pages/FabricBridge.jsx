    ...
    <>
      ...
      <BrowserWin url="localhost:8080" toast={toast}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <div className="tile" style={{ '--c': '#34d399' }}><small>1 · Connect</small><div style={{ marginTop: 7 }}><span className="p ok">APS</span> <span className="p ok">Fabric</span></div></div>
          <div className="tile" style={{ '--c': '#6ea8fe' }}><small>2 · Models selected</small><b style={{ fontSize: 24 }}>{nSel}</b></div>
          <div className="tile" style={{ '--c': '#38bdf8' }}><small>3 · Destination</small><div style={{ marginTop: 7 }}><span className="p in">OneDrive → OneLake</span></div></div>
        </div>
        <div className="card">
        ...
        <div className="card" style={{ flex: 1 }}>
          ...
          {files.map((f) => (
            <div className="pr" key={f.id}>
              ...
              <div className="bar"><i style={{ width: (f.sel ? f.p : 0) + '%', background: f.p >= 100 ? '#34d399' : '#38bdf8', transition: 'width .3s' }} /></div>
